...
with tab_monitor:
    ...
    if langsmith_ok:
        st.caption(
            f"Observabilidad via **LangSmith** — proyecto: `{langsmith_project}` · "
            f"[Abrir dashboard ↗](https://smith.langchain.com)"
        )
    else:
        st.warning(
            "LangSmith no está configurado. Ve a **Configuración** y agrega tu API Key "
    ...
    if daily:
        ...
        fig_cost = px.area(
            df_daily, x="date", y="cost_usd", color_discrete_sequence=["#f59e0b"]
        )
        fig_cost.update_layout(
            margin=dict(t=10, b=10), height=220, xaxis_title="", yaxis_title="Costo USD"
        )
    ...
    if not active_alerts:
        ...
        for alert in active_alerts:
            ...
            st.warning(
                f"{level_icon} **[{alert['category'].upper()}]** {alert['message']}  \n"
    ...
    if alert_history:
        with st.expander(
            f"📜 Historial de alertas ({len(alert_history)} últimos 7 días)"
        ):
            for ah in alert_history[:20]:
                status_icon = "✅" if ah["resolved"] else "🔴"
                st.caption(
                    f"{status_icon} [{ah['level']}] {ah['category']}: {ah['message']} — {ah['timestamp'][:19]}"
                )
    ...
    if sec_events:
        with st.expander("🔍 Últimos eventos de seguridad"):
            for ev in sec_events:
                ...
                st.caption(
                    f"{blocked_tag} [{ev['severity']}] {ev['event_type']}: {ev['details']} — {ev['timestamp'][:19]}"
                )
    ...
    if not active_incidents:
        ...
        for inc in active_incidents:
            ...
            st.error(
                f"{sev_color} **{inc['title']}** — Estado: {inc['status']}  \n"
    ...
    if incident_history:
        with st.expander(f"📋 Historial de incidentes ({len(incident_history)})"):
            for ih in incident_history:
                status_icon = (
                    "✅" if ih["status"] in ("resolved", "postmortem") else "🔄"
                )
                cause = f" — Causa: {ih['root_cause']}" if ih.get("root_cause") else ""
                st.caption(
                    f"{status_icon} [{ih['severity']}] {ih['title']}{cause} — {ih['created_at'][:19]}"
                )
    ...
    if not langsmith_ok:
        ...
        if not recent_traces:
            st.info(
                "Aún no hay trazas. Se registrarán automáticamente al analizar autos."
            )
        else:
            ...
            st.dataframe(
                df_traces[available], hide_index=True, use_container_width=True
            )
...
with tab_cfg:
    ...
    with st.expander(
        ...
        expanded=not bool(os.getenv("LANGSMITH_API_KEY")),
    ):
        st.markdown(
        ...
        new_ls_key = _secret_input(
            "LangSmith API Key", os.getenv("LANGSMITH_API_KEY"), "cfg_ls_key"
        )
        new_ls_project = st.text_input(
            "Nombre del proyecto",
            value=os.getenv("LANGSMITH_PROJECT", "VentasAutomatizacion"),
            key="cfg_ls_project",
        )
